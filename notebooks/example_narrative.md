...
## Ingest And Summarize Notebook Parity

An end-to-end notebook example is available for document ingest + concept-in-source
summary generation + KB logging using the same backend used by the UI.

See [this notebook](example_analytics_ingest_summarize_parity.ipynb), which uses the
UCI incident management source URL and logs an attribute summary as an exploratory
observation.

The same notebook also includes a search-then-edit example that finds an observation
with the snapshot search API and updates it in the KB, then verifies persistence.
